...
def BuyTicket():
    try:
        ...
        if not seat_input.strip() or not seat_input.isdigit():
          print("\n 🚨 Kindly enter a valid numeric seat number between 1 and 40.")
          return

        seat_number = int(seat_input)
   
        if(seat_number > 40 or seat_number <= 0 or seat_number == ''):
